...
var numberByUser = prompt('Ingresa una número para buscar por nota')
var parsedNumberByUser = parseInt(numberByUser, 10)
searchStudentByAverageGrade(parsedNumberByUser)

var name = prompt('Ingresa el nombre o apellido del alumno')
searchStudentByName(name)

function searchStudentByAverageGrade (averageGrade) {
  for (var i = 0; i < students.length; i++) {
    var student = students[i]
    if (averageExamResult(student.examResults) >= averageGrade) {
...
function searchStudentByName (studentName) {
  for (var i = 0; i < students.length; i++) {
    ...
    if (student.firstName === studentName) {
      showStudentDetail(student)
      break
    } else if (student.lastName === studentName) {
      showStudentDetail(student)
      break
    }
  }
